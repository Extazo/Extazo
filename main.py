...
async def general(ctx):
    ...
    embed.add_field(name=' `$end`', value='Éteint le selfbot', inline=False)
    embed.add_field(name=' `$copy`', value='Crée une back up', inline=False)
    embed.add_field(name=" `$snipe`", value='Affiche un message supprimé', inline=False)
    embed.set_footer(text='Crée par Extazy ')
...
async def info(ctx):
    ...
    embed.add_field(name=' `$banner <server id>`', value='Grab la bannière d un serveur', inline=False)
...
async def status(ctx):
    ...
    embed.add_field(name='`$play`', value='Active le statut joue à', inline=False)
    embed.add_field(name='`$listen`', value='Active le statut écouter', inline=False)
    embed.add_field(name='`$watch`', value='Active le statut regarde', inline=False)
    embed.add_field(name='`$removestatus`', value='Retire le status personnalisé', inline=False)
    embed.set_footer(text='Crée par Extazy')
    await ctx.send(embed=embed)
...
async def help(ctx):
    ...
    embed.set_thumbnail(url=(client.user.avatar_url))
    embed.set_image(url='https://media.discordapp.net/attachments/918266987459784785/932250024040083466/20220116_132625.gif')
    embed.add_field(name='`$general`', value='Affiche le menu général', inline=False)
    ...
    embed.add_field(name='`$moderation`', value='Affiche le menu modération', inline=False)
...
async def nuke(ctx):
    ...
    embed.add_field(name='`$rr`', value='Rename all rôles sur un serveur', inline=False)
    ...
    embed.add_field(name=' `$adminall`', value='Give le rôles admin a tous le monde', inline=False)
    embed.set_footer(text='Crée par Extazy')
    await ctx.send(embed=embed)
...
async def prefix(ctx, prefix):
    ...
    await ctx.message.delete()
    await ctx.send('```Le préfixe a bien été changé```')
...
async def unmute(ctx, member: discord.Member):
    mutedRole = discord.utils.get((ctx.guild.roles), name='340 tas muté')
    await member.remove_roles(mutedRole)
    await member.send(f" Tu as été démuté: - {ctx.guild.name}")
...
async def mute(ctx, member: discord.Member, *, reason=None):
    guild = ctx.guild
    mutedRole = discord.utils.get((guild.roles), name='340 ta muté')
    if not mutedRole:
        mutedRole = await guild.create_role(name='340 mute')
        for channel in guild.channels:
            await channel.set_permissions(mutedRole, speak=False, send_messages=False, read_message_history=True, read_messages=False)

    embed = discord.Embed(title='340 - Selfbot', description=f"{member.mention} as été mute ", colour=(discord.Colour.light_gray()))
    embed.add_field(name='Raison :', value=reason, inline=False)
    await ctx.send(embed=embed)
    await member.add_roles(mutedRole, reason=reason)
    await member.send(f" Tu as été mute par : {guild.name} Raison : {reason}")
...
async def raid(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason='340 à niquer le serveur')
        except:
            pass
    ...
    for user in users:
        try:
            await user.ban(reason='340 à niquer le serveur')
        except:
            pass
...
async def kick(ctx, member: discord.Member, *, reason=None):
    ...
    await ctx.send('```Utilisateur kick avec succès``')
...
async def ban(ctx, member: discord.Member, *, reason=None):
    ...
    await ctx.send('```Utilisateur bannit avec succès```')


@client.command()
async def lock(ctx):
    await ctx.channel.set_permissions((ctx.guild.default_role), send_messages=False)
    await ctx.send(ctx.channel.mention + 'Permissions bloqué')
...
async def adminall(ctx):
    ...
    try:
        ...
        print(Fore.MAGENTA + '**Give rôle admin...**' + Fore.RESET)
    except:
        print(Fore.GREEN + '**Give avec succès' + Fore.RESET)


@client.command()
async def unlock(ctx):
    await ctx.channel.set_permissions((ctx.guild.default_role), send_messages=True)
    await ctx.send(ctx.channel.mention + 'Permissions débloqué')
...
async def leave(ctx, guild_id):
    ...
    await ctx.send(f"Tu as quitté {guild_id}")
...
@client.command()
async def alive(ctx):
    embed = discord.Embed(title='340 - Selfbot', description='340 Selfbot est toujours connecté faite $help')
    embed.set_footer(text='Crée par Extazy ')
    await ctx.send(embed=embed)
